...
describe('Post workflow', () => {
  ...
  describe('Super Admin review', () => {
    ...
    beforeEach(() => { service = new PostAdminService({} as never, dataSource as never, audit as never); });

    it('duyệt bài pending thành published', async () => {
      repo.findOne.mockResolvedValue({ id: '5', status: PostStatus.Pending, translations: [{ title: 'Post' }] });
      const result = await service.approve(actor, '5', '127.0.0.1');
    ...
    it('không duyệt bài không ở trạng thái pending', async () => {
      repo.findOne.mockResolvedValue({ id: '5', status: PostStatus.Draft, translations: [] });
      await expect(service.approve(actor, '5', '127.0.0.1')).rejects.toBeInstanceOf(BadRequestException);
    });

    it('hủy duyệt đưa published về pending và xóa thông tin review', async () => {
      repo.findOne.mockResolvedValue({ id: '5', status: PostStatus.Published, reviewedBy: 1, reviewedAt: new Date(), publishedAt: new Date(), translations: [{ title: 'Post' }] });
  ...
  describe('Blog Owner submit', () => {
    ...
    beforeEach(() => {
      service = new PostOwnerService({} as never, dataSource as never, audit as never);
    });
    const owner = { ...actor, role: RoleName.BlogOwner };

    it('gửi draft có translation sang pending', async () => {
      repo.findOne.mockResolvedValue({ id: '9', authorId: 1, status: PostStatus.Draft, rejectionReason: null, translations: [{ title: 'Draft' }] });
    ...
    it('không gửi bài thiếu translation', async () => {
      repo.findOne.mockResolvedValue({ id: '9', authorId: 1, status: PostStatus.Draft, translations: [] });
