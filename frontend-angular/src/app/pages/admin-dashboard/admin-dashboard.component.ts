import { AfterViewInit, ChangeDetectionStrategy, Component, computed, CUSTOM_ELEMENTS_SCHEMA, effect, ElementRef, inject, OnDestroy, signal, ViewChild, ViewEncapsulation } from '@angular/core';
...
export class AdminDashboardComponent implements AfterViewInit, OnDestroy {
  private readonly api=inject(DashboardApiService);private readonly notifications=inject(NotificationService);
  ...
  private readonly data=signal<DashboardData|null>(null);private readonly events:EventSource;
  ...
  protected readonly topPosts=computed(()=>this.data()?.topPosts.map(post=>{const title=post.translations.find(item=>item.languageId===this.language.languageId())?.title??post.translations[0]?.title??`#${post.id}`;return{id:Number(post.id),title,likes:post.likesCount,comments:post.commentsCount,value:post.likesCount+post.commentsCount};})??[]);
  ...
  constructor(){effect(()=>this.load(this.period()),{allowSignalWrites:true});effect(()=>{this.language.locale();this.engagementChartData();queueMicrotask(()=>this.renderChart());});this.events=new EventSource(`${APP_CONFIG.apiBaseUrl}/admin/dashboard/events`,{withCredentials:true});this.events.onmessage=(message)=>{try{const event=JSON.parse(message.data) as {type?:string};if(event.type==='LIKE_CHANGED'||event.type==='COMMENT_CREATED')this.load(this.period());}catch{this.load(this.period());}};}
  ...
  private renderChart():void{const canvas=this.canvas?.nativeElement;if(!canvas)return;const rows=this.engagementChartData();const config:ChartConfiguration<'line'>={type:'line',data:{labels:rows.map(row=>row.date.slice(5)),datasets:[{label:this.language.choose('Lượt thích','Likes'),data:rows.map(row=>row.likes),borderColor:'#168b20',backgroundColor:'rgba(22,139,32,.12)',pointBackgroundColor:'#168b20',pointRadius:3,pointHoverRadius:6,tension:.32,fill:false},{label:this.language.choose('Bình luận','Comments'),data:rows.map(row=>row.comments),borderColor:'#7357d8',backgroundColor:'rgba(115,87,216,.12)',pointBackgroundColor:'#7357d8',pointRadius:3,pointHoverRadius:6,tension:.32,fill:false}]},options:{responsive:true,maintainAspectRatio:false,animation:{duration:250},interaction:{mode:'index',intersect:false},plugins:{legend:{display:false},tooltip:{enabled:true}},scales:{x:{grid:{display:false},ticks:{maxTicksLimit:6}},y:{beginAtZero:true,ticks:{precision:0},grid:{color:'rgba(127,127,127,.15)'}}}}};if(this.chart){this.chart.data=config.data;this.chart.options=config.options??{};this.chart.update();}else this.chart=new Chart(canvas,config);}
